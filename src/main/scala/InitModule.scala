...
class InitModule[F[_]: Async](implicit contextShift: ContextShift[F], implicit val concurrent: Concurrent[F]) extends ConfigModule {
  logConfig()
  ...
  private lazy val transactor: Transactor[F] = Transactor.fromDriverManager[F](driver, connectionString, config.dbUser, config.dbPassword)
